...
public class pagination {
	...
	public static void main(String[] args)
	{
		...
		//for that
		//scan the name column with getText and -> Wheat and print the price of the Wheat
		List<String> price; // i am declaring here... so that we can use for while loop.... if we dont declare here then "price"variable will be local only for method "do"
		do
		{
			List<WebElement> newElements = driver.findElements(By.xpath("//tr/td[1]")); // here we are going to get the elements in the table....
			//we have declared first itself...but this will collect after moving to next page also
		 price = newElements.stream().filter(s->s.getText().contains("Wheat")).map(s->getpriceVeggie(s)).collect(Collectors.toList());
		
		price.forEach(a->System.out.println(a)); // we are printing the price finally
		// above method explanation
		// we are taking the "newElements" list which we got from line 38
		// we are converting it to stream
		// from that stream we are getting only the element containing text "Wheat"
		// now the stream contain only the webelement of "Wheat"
	...
	private static String getpriceVeggie(WebElement s) { // here we are writing this method to catch the price of the Wheat which is given in the next column
